...
class MemberService(
    ...
    fun getMemberInfo(id: Long) = MemberInfoResponse.from(memberRepository.findByIdOrThrow(id, "존재하지 않는 회원입니다."))
    ...
    @Transactional
    fun updateMemberToStylist(id: Long, request: MemberUpdateRequest): MemberUpdateStylistResponse {
        val member = memberRepository.findByIdOrNull(id)?.takeIf { encoder.matches(request.password, it.password) }
            ?: throw IllegalArgumentException("아이디 또는 비밀번호가 일치하지 않습니다.")
